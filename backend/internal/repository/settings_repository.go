...
func (r *SettingsRepository) Get(companyID string) (*models.Settings, error) {
    ...
    query := `SELECT id, center_name, logo, theme_color, timezone, company_id FROM settings WHERE company_id = $1 LIMIT 1`

    var tz string
    err := r.db.QueryRow(query, companyID).Scan(&settings.ID, &settings.CenterName, &logo, &settings.ThemeColor, &tz, &settings.CompanyID)
	if err == sql.ErrNoRows {
        ...
        insertQuery := `
            INSERT INTO settings (center_name, logo, theme_color, timezone, company_id)
            VALUES ($1, $2, $3, $4, $5)
            RETURNING id
        `
        err = r.db.QueryRow(insertQuery, defaultSettings.CenterName, defaultSettings.Logo, defaultSettings.ThemeColor, defaultSettings.Timezone, defaultSettings.CompanyID).Scan(&defaultSettings.ID)
		if err != nil {
...
func (r *SettingsRepository) Update(settings *models.Settings, companyID string) error {
	...
	if err == sql.ErrNoRows {
        ...
        insertQuery := `
            ...
            RETURNING id
        `
        err = r.db.QueryRow(insertQuery, settings.CenterName, settings.Logo, settings.ThemeColor, settings.Timezone, companyID).Scan(&settings.ID)
		if err != nil {
        ...
        `
        result, err := r.db.Exec(updateQuery, settings.CenterName, settings.Logo, settings.ThemeColor, settings.Timezone, existingID, companyID)
		if err != nil {
